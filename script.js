...
const PD_DATES=[['2026-08-28',false],['2026-08-31',false],['2026-10-07',true],['2026-12-02',true],['2027-02-03',true],['2027-03-03',true],['2027-05-05',true]];
const DMS_EARLY_DISMISSALS=[['2026-09-18',''],['2026-10-07',''],['2026-11-09','DMS Conferences'],['2026-11-13',''],['2026-11-25',''],['2026-12-02',''],['2026-12-23',''],['2027-02-03',''],['2027-02-11','DMS Parent Conferences'],['2027-02-12',''],['2027-03-03',''],['2027-05-05',''],['2027-06-11','Tentative'],['2027-06-14','Tentative'],['2027-06-15','Tentative last day']];
...
function nextDatedEvent(today,events){return events.find(([date])=>date>=today)||null;}
...
if(typeof module!=='undefined')module.exports={CALENDAR,PD_DATES,DMS_EARLY_DISMISSALS,day,iso,eligible,validSnow,finalDay,schoolCount,nextDatedEvent,calculate,todayNY};
...
const fullDate=s=>new Intl.DateTimeFormat('en-US',{month:'long',day:'numeric',timeZone:'UTC'}).format(new Date(s+'T00:00:00Z'));
function render(){const today=todayNY(),v=calculate(today,snow);for(const id of ['calendar','school','untilCalendar','untilSchool'])$(id).textContent=v[id];$('snow').textContent=snow;$('holiday').textContent=v.complete?'School year complete':v.event?(v.ongoing?'NOW: ':'')+v.event[0]:'No more scheduled days off';const e=v.event;$('date').textContent=e?short(v.ongoing?today:e[1])+(e[2]&&e[2]!==today?'–'+short(e[2]):''):'';const pd=nextDatedEvent(today,PD_DATES),early=nextDatedEvent(today,DMS_EARLY_DISMISSALS);$('pd-date').textContent=pd?fullDate(pd[0]):'None remaining';$('pd-note').textContent=pd&&pd[1]?'Early Dismissal':'';$('early-date').textContent=early?fullDate(early[0]):'None remaining';$('early-note').textContent=early?early[1]:'';$('undo').disabled=snow===0;$('reset').disabled=snow===0;$('add').disabled=snow>=1000;$('status').textContent=storageOK?'':'Snow-day changes cannot be saved in this browser.';}
